    ...
    <div className="space-y-16">
      <header className="animate-reveal-up opacity-0" style={{animationFillMode: 'forwards'}}>
        <div className="flex items-center gap-4 mb-6">
          <span className="section-num">{t.calendar.section}</span>
          <div className="rule flex-1"></div>
        </div>
        <div className="flex items-end justify-between">
          <h1 className="text-huge">
            {t.calendar.title1} <span className="italic font-light text-terracotta">{t.calendar.title2}</span>{t.calendar.title3}
          </h1>
          <button className="btn-editorial group">
            <Plus className="w-4 h-4" />
            <span>{t.calendar.schedulePost}</span>
          </button>
        </div>
      </header>
      ...
      <section className="editorial-frame p-8 animate-reveal-up opacity-0 delay-200" style={{animationFillMode: 'forwards'}}>
        <div className="flex items-center justify-between mb-8 pb-6 border-b border-ink/10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="text-eyebrow text-terracotta">{t.calendar.calendarIssue}</div>
              <h2 className="text-3xl font-black tracking-tight">{t.calendar.calendarMonth}</h2>
            </div>
            <button className="w-10 h-10 border border-ink/20 hover:bg-ink hover:text-ivory hover:border-ink transition-all flex items-center justify-center">
      ...
      <section className="animate-reveal-up opacity-0 delay-300" style={{animationFillMode: 'forwards'}}>
        <div className="mb-8">
          <div className="section-num mb-2">{t.calendar.thisWeek}</div>
          <h2 className="text-4xl font-black tracking-tight">{t.calendar.goingOut}</h2>
        </div>
          ...
          {mockScheduledPosts.map((post, i) => {
            ...
            return (
              ...
              >
                ...
                <div className={`flex-1 ${dir === 'rtl' ? 'group-hover:-translate-x-2' : 'group-hover:translate-x-2'} transition-transform duration-500`}>
